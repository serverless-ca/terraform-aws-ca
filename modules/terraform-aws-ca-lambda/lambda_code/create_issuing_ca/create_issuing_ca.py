...
def lambda_handler(event, context):  # pylint:disable=unused-argument
    ...
    root_ca_name = ca_name(cfg.project, cfg.environment_name, "root")
    ca_slug = ca_name(cfg.project, cfg.environment_name, "issuing")

    # check Root CA exists
    if not db_list_certificates(cfg.project, cfg.environment_name, root_ca_name):
    ...
    root_ca_cert_pem = base64.b64decode(
        db_list_certificates(cfg.project, cfg.environment_name, root_ca_name)[0]["Certificate"]["B"]
    )
    ...
    s3_upload(
        cfg.external_s3_bucket,
        cfg.internal_s3_bucket,
        cert_bundle_pem,
        f"{ca_bundle_name(cfg.project, cfg.environment_name)}.pem",
    )
